...
WHERE EXISTS (SELECT id FROM word as w where w.id = t.id AND ld_ratio(SOUNDEX(@word), SOUNDEX(misspelled_word)) >= 55)
)
SELECT * 
FROM CTE
WHERE EXISTS (SELECT id FROM word as w where w.id = CTE.id AND (ld_ratio(@word, misspelled_word) >= 58 OR ld(@word,misspelled_word) < 5); 
