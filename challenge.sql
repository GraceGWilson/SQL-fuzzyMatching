...
FROM ( SELECT *, SUBSTR(misspelled_word,1,2) AS mf1, SUBSTR(misspelled_word,2,2) AS mf2, 
	...
	FROM word
	HAVING  dm(@word) LIKE dm(misspelled_word),
		...
		SUBSTR(REVERSE(@word),6,2) IN (mmr4, mr5, mr6, mr7, mr8) OR
...
WHERE EXISTS (SELECT id FROM word as w where w.id = t.id AND ld_ratio(SOUNDEX(@word), SOUNDEX(misspelled_word)) >= 70); 
