...
FROM ( SELECT *, SUBSTR(misspelled_word,1,2) AS mf1, SUBSTR(misspelled_word,2,2) AS mf2, 
      		...
      		SUBSTR(misspelled_word,9,2) AS mf9,
      		...
      		SUBSTR(REVERSE(misspelled_word),9,3) AS mr9
	FROM word
	HAVING @word SOUNDS LIKE misspelled_word OR
		...
		SUBSTR(@word,7,2) IN (mf5, mf6, mf7,mf8,mf9) OR
      		SUBSTR(@word,8,2) IN (mf5, mf6, mf7,mf8,mf9) OR
      		SUBSTR(@word,9,2) IN (mf5, mf6, mf7,mf8,mf9) OR
		...
		SUBSTR(REVERSE(@word),7,3) IN (mr5, mr6, mr7, mr8, mr9) OR
      		...
      		SUBSTR(REVERSE(@word),9,3) IN (mr5, mr6, mr7, mr8, mr9)
	 ) AS t 
